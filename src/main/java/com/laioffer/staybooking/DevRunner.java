...
//
//// Spring Boot 运行流程：
//// 应用启动，Spring Boot 加载所有 Bean。
//// DevRunner 被 Spring 扫描到 并注册为 Bean （因为有 @Component）。
//// Spring Boot 完成启动，此时 ApplicationRunner.run() 触发。
//// 执行 DevRunner.run() 方法，从而调用 generateSampleData()，插入测试数据，并进行数据库查询。
//
//// ApplicationRunner 是 Spring Boot 提供的一个启动后自动执行的回调接口，
//// 其 run(ApplicationArguments args) 方法会在 Spring Boot 应用启动完成后 自动执行。
//// 所以 DevRunner 会在启动时自动执行
//@Component
...
//                        geometryFactory.createPoint(new Coordinate(-122.4787, 37.8199)) // 这个经纬度是反过来的
//                ),
//                new ListingEntity(
//                        null,
//                        1L,
